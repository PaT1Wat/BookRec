    ...
    <div className="container py-8">
      ...
      <div className="grid gap-8 md:grid-cols-[280px_1fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`h-5 w-5 ${i < Math.round(avgRating) ? "fill-star text-star" : "text-muted-foreground/30"}`} />
              ))}
            </div>
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed text-muted-foreground">{book.description || "ไม่มีคำอธิบาย"}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {genres.map((g) => (
                <Link key={g} to={`/search?genre=${g}`} className="rounded-full bg-tag-bg px-3 py-1 text-xs font-medium text-tag-fg hover:bg-primary/10 transition-colors">{g}</Link>
              ))}
              {tags.map((t) => (
      ...
      <section className="mt-12">
        ...
        {!user ? (
          <div className="rounded-2xl border border-border bg-card p-6 text-center text-muted-foreground">
            <Link to="/auth" className="text-primary hover:underline">เข้าสู่ระบบ</Link> เพื่อเขียนรีวิว
          </div>
        ) : (
          <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-1">
                ...
                {myRating > 0 && (
                  <span className="ml-2 self-center text-sm text-muted-foreground">{myRating}/5</span>
                )}
              </div>
            </div>
      ...
      <section className="mt-8">
        <h2 className="mb-6 text-xl font-bold text-foreground font-display">
          💬 รีวิวทั้งหมด
          {reviews.length > 0 && (
            <span className="ml-2 text-sm font-normal text-muted-foreground">({reviews.length} รีวิว)</span>
          )}
        </h2>
